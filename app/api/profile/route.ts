...
export async function GET(req: Request) {
  try {
    ...
    return NextResponse.json({ message: "Profile Fetch Error: " + (error.message || "Unknown error") }, { status: 500 });
  }
}
...
export async function PUT(req: Request) {
  try {
    ...
    return NextResponse.json({ message: "Profile Update Error: " + (error.message || "Server Error") }, { status: 500 });
  }
}
