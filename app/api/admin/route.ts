...
export async function GET(req: Request) {
  try {
    ...
    return NextResponse.json({ message: "Admin GET Error: " + (error.message || "Unknown error") }, { status: 500 });
  }
}
...
export async function PUT(req: Request) {
  try {
    ...
    return NextResponse.json({ message: "Admin PUT Error: " + (error.message || "Server Error") }, { status: 500 });
  }
}
